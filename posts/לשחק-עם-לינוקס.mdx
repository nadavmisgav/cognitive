...
הko
ביצעתי שגיאות/נסיונות, תקראו לזה איך שבא לכם, ובמקרה הטוב זה יצר לי
kernel module
...
2. מערכת קבצים - ארצה ליצור מערכת קבצים אשר תכיל בינארים בסיסיים לעבודה עם linux.
...
של trovalds, הורדתי את זה לתוך תקיית `/opt` מכיוון שזה קונבציה.
```bash
cd /opt
sudo mkdir linux
...
הדרך שבה נהוג לשנות את הקובץ קונפוגרציה היא לא על ידי עריכה ישירה אלא על ידי חלון קינפוג שאותו מריצים כך,
...
זה פשוט יותר זה כי אחרת בקימפולים השונים הייתיצריך לבצע
cross-compile
...
```
כעת ניתן לקמפל כאשר גם כאן בדומה לקרנל יש דיי הרבה קונפוגרציות אפשרויות לכן תחילה קימפלתי עם הגדרות ברירת המחדל,
לאחר מכן השתמשתי
ב`menuconfig`
...
## הרצת qemu
געת יש לי את כל הקבצים שאני צריך וניתן להריץ את ההפצת
linux
החדשה על ידי
...
במהלך הקימפולים צריך כל מיני בינארים וספריות, זה מה שאני הייתי צריך.
