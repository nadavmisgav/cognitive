...
def read_message(future_result: Future[str]) -> None:
    message = _read(...)
...
deadlock,
לכן היינו חייבים להעציל את המשימה
לthread
...
במקרים כאלו נרצה גם להעציל את המשימה הזאת
לthread
אחר ובדיוק לזה יצרו את פונקציה הבאה לנוחות
`asyncio.run_in_executor`
אשר מקבלת פונקציה סינכרונית, מעצילה
לthread
אחר ומחזירה לנו
Future
...
בפוסט הזה צללנו לעבודה האסינכרונית
בPython
עם
`asyncio`.
...
`trio`
שהיא ספריה שמנסה לכתוב בצורה פשוטה יותר ופחות מקום לשגיאות את הניהול של קוד אסינכרוני.
