...
ונצטרך להוסיף לקובץ
`Parser/Python.asdl`
שמגדיר את העץ את התיאור של מה זה הפרטמר שהוספנו לפונקציה 
...
מגניב 🎉 עכשיו ניתן לקמפל את הפייתון הזה ויש לנו
Syntax
חדש, אבל כרגע הוא לא עושה כלום - רק לא זורק לכם
`SyntaxError`.
...
ה`except`
נבדוק האם קיים תנאי עבור הטיפול בנוכחי בשגיאה
(ה`struct` שאני משתמש בו למעשה נוצר דינאמית לפי הAST)
כי לא חייב להיות תנאי, ואם הוא קיים אז נבדוק אותו.
במקרה והתנאי מתקיים ממשיכים כרגיל השאר הקוד,
אם התנאי לא מתקיים
(זה ה0 בסוף הפונקציה)
אז נקפוץ
לlabel
...
#### except עם השמה
כעת ארצה לטפל בקטע קוד שגם מבצע השמה,
```python
except Exception as e: ...
```
זה גם לוגיקה שמטופלת בפונקציה
`compiler_try_except`
שערכתי אך שם זה קצת יותר מסובך.
...
`Store`
למשתנה. איך שזה עובד בפייתון זה שהמשתנים נמצאים
בstack
...
>>> 
```
כדי לשחזר את זה אני מוסיף את השינויים שעשיתי
